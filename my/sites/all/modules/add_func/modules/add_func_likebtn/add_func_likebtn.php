...
class Add_Func_Likebtn {
	...
	public function runSyncVotes() {
		if ( !self::$synchronized && variable_get( 'add_func_likebtn_account_data_email' ) && variable_get( 'add_func_likebtn_account_data_api_key' ) && $this->timeToSyncVotes( variable_get( 'add_func_likebtn_sync_inerval', 60 ) * 60 ) && function_exists( 'curl_init' ) ) {
			$this->syncVotes( variable_get( 'add_func_likebtn_account_data_email' ), variable_get( 'add_func_likebtn_account_data_api_key' ) );
		}
	}
	...
	public function timeToSyncVotes( $sync_period ) {
		$last_sync_time = variable_get( 'add_func_likebtn_last_sync_time', 0 );

		$now = time();
		if ( !$last_sync_time ) {
			variable_set( 'add_func_likebtn_last_sync_time', $now );
	...
	public function curl( $url ) {
		$path = drupal_get_path( 'module', 'add_func_likebtn' ) . '/add_func_likebtn.info';
		$info = drupal_parse_info_file( $path );
		$drupal_version = VERSION;
		$add_func_likebtn_version = $info["core"];
		$php_version = phpversion();
		$useragent = "Drupal $drupal_version; likebtn module $add_func_likebtn_version; PHP $php_version";
	...
	public function syncVotes( $account_api_key, $site_api_key ) {
		...
		$last_sync_time = number_format( variable_get( 'add_func_likebtn_last_sync_time', 0 ), 0, '', '' );

		$updated_after = '';

		if ( variable_get( 'add_func_likebtn_last_successfull_sync_time', 0 ) ) {
			$updated_after = variable_get( 'add_func_likebtn_last_successfull_sync_time' ) - ADD_FUNC_LIKEBTN_LAST_SUCCESSFULL_SYNC_TIME_OFFSET;
		}
		...
		if ( $sync_result ) {
			variable_set( 'add_func_likebtn_last_successfull_sync_time', $last_sync_time );
		}
	}
	...
	public function updateVotes( $response ) {
		...
		if ( !empty( $response['response']['items'] ) ) {
			foreach ( $response['response']['items'] as $item ) {
				...
				$vote_source = ADD_FUNC_LIKEBTN_VOTING_VOTE_SOURCE;
				if ( $field_id ) {
				...
				$votes[] = array(
					...
					'value_type'	=> 'points',
					'value'			=> $likes,
					'tag'			=> ADD_FUNC_LIKEBTN_VOTING_TAG,
					'uid'			=> 0,
				...
				if ( $vote_source ) {
					...
					foreach ( $entity_fields as $field_name => $field_info ) {
						if ( $field_info['widget']['module'] != 'add_func_likebtn' ) {
							continue;
						}
						...
						$field_fields_data[$field_name . '_add_func_likebtn_likes'] = $likes;
						$field_fields_data[$field_name . '_add_func_likebtn_dislikes'] = $dislikes;
						$field_fields_data[$field_name . '_add_func_likebtn_likes_minus_dislikes'] = $likes_minus_dislikes;
	...
	/*public function apiRequest( $action, $request, $email = '', $api_key = '' ) {
		if ( !self::$apiurl ) {
			...
			if ( !$api_key ) {
				$api_key = trim( variable_get( 'add_func_likebtn_account_data_api_key' ) );
			}
			$subdirectory = trim( variable_get( 'add_func_likebtn_settings_subdirectory' ) );
			$local_domain = trim( variable_get( 'add_func_likebtn_settings_local_domain' ) );
			if ( $local_domain ) {
				$domain = $local_domain;
			} else {
			...
			self::$apiurl = ADD_FUNC_LIKEBTN_API_URL . "?email={$email}&api_key={$api_key}&domain={$domain}&nocache=.php&source=drupal&";
		}
